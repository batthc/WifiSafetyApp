...
def get_hmac_secret() -> bytes:
    """
    Loads secret once per warm Lambda container.
    Secret value should be a plain string.
    """
...
def hmac_fingerprint(ssid: str, bssid: Optional[str], country: Optional[str]) -> str:
    ...
    # privacy-friendly: normalize and include only coarse components
...
def compute_score(req: ScanRequest, rep_seen: int, rep_high_rate: float):
    ...
    # Keep crowd reputation impact small to reduce poisoning
    ...
    # return top 3 biggest negatives
    ...
    if label == "HIGH":
        ...
        advice = [
            "Lower risk based on checks. Still prefer HTTPS; VPN is optional."
        ]
